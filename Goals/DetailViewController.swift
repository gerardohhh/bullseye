...
class DetailViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource, MediaCellDelegate, UIViewControllerTransitioningDelegate, AVPlayerViewControllerDelegate, AVAudioPlayerDelegate {
...
//          _ = peekPop?.registerForPreviewingWithDelegate(self, sourceView: collectionView)
     ...
     override func viewDidAppear(_ animated: Bool) {
          ...
          if let updateImage = currentUpdate?["image"] as? PFFile {
               media.append(["type": "photo", "image": updateImage, "sender": currentUpdate?["author"]! as Any, "caption": updateLabel.text!, "rotated": true])
               collectionView.reloadData()
          }
     }
